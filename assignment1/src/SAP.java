...
public class SAP
{
     ...
     *         ancestral path; -1 if no such path exists
     */
     ...
     * @param w
     *
     * @return length of shortest ancestral path between any vertex in v and
     * any vertex in w; -1 if no such path exists
     */
    ...
    private SAPProcessor cachedResult(int v, int w)
    {
        ...
        if (cache.containsKey(key))
        {
            ...
            // We need to cache only for 2 consecutive calls, therefore we
            // delete the result after the second call.
            cache.remove(key);
    ...
    private class SAPProcessor
    {
        ...
        private void process(BreadthFirstDirectedPaths a,
                             BreadthFirstDirectedPaths b)
        {
            ...
            {
                if (a.hasPathTo(i) && b.hasPathTo(i)) ancestors.add(i);
            }

            int shortestAncestor = -1;
            int minDistance = Integer.MAX_VALUE;

            for (int anAncestor : ancestors)
            {
                int dist = a.distTo(anAncestor) + b.distTo(anAncestor);

                if (dist < minDistance)
                {
                    minDistance = dist;
                    shortestAncestor = anAncestor;
                }
            }
     ...
     *
     * @param args path to a diagraph file
     */
